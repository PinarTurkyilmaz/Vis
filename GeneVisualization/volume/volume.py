...
class GradientVolume:
    ...
    def compute(self):
        ...
        for i in range(0,self.volume.dim_x-1,1):
            for j in range(0, self.volume.dim_y-1,1):
                for k in range(0, self.volume.dim_z-1,1):
                    delta_x =(1/2*(self.volume.get_voxel(i+1,j,k)-self.volume.get_voxel(i-1,j,k)))

                    delta_y = (1/2*(self.volume.get_voxel(i,j+1,k)-self.volume.get_voxel(i,j-1,k)))

                    delta_z= (1/2*(self.volume.get_voxel(i,j,k+1)-self.volume.get_voxel(i,j,k-1)))

                    self.set_gradient(i,j,k,VoxelGradient(delta_x,delta_y,delta_z))
    ...
    #     gradient
    # self.set_gradıent(ı,j,k,VoxelGradient(derıv_x,deriv_y,deriv_z))
